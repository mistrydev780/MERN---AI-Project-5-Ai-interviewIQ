...
export const finishInterview = async (req, res) => {
  try {
    ...
    console.log(error)
    return res.status(500).json({ message: `failed to submit answer Interview ${error}` })
  }
}


export const getMyInterviewas = async (req, res) => {
  try {
    const interviews =
 await Interview.find({ userId: req.userId })
      .sort({ createdAt: -1 })
   .select(
 "role experience mode finalScore status createdAt"
    ...
    return res.status(200).json({ message: `failed to find CurrentUser Interview ${error}` })
  }
}
